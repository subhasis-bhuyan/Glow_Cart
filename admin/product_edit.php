<?php
/**
 * GlowCart Cosmetics - Edit Product Form & Handler
 */
require_once __DIR__ . '/auth_check.php';

$admin_page_title = 'Edit Product | GlowCart Admin';
$admin_header_title = 'Edit Product';
$active_tab = 'products';
...
if (!$product) {
    $_SESSION['flash_error'] = "Product #{$product_id} was not found in the catalog.";
    header('Location: products.php');
    exit;
}
...
$is_bestseller = (int)$product['is_bestseller'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $image = clean_input($_POST['image'] ?? $product['image']);
    ...
    if (empty($errors)) {
        try {
            ...
            $_SESSION['flash_success'] = "Product #{$product_id} ('{$name}') was updated successfully.";
            header('Location: products.php');
            exit;
...
<div class="page-header">
    <div>
        <h1 class="page-title">Edit Product: <?= htmlspecialchars($product['name']) ?></h1>
        <p style="color: var(--admin-muted); font-size: 13px; margin-top: 4px;">Update inventory, pricing, images, or status for product #<?= $product_id ?>.</p>
    </div>
    <div style="display: flex; gap: 10px;">
        <a href="../product_details.php?id=<?= $product_id ?>" target="_blank" class="admin-btn admin-btn-outline">
            <span>👁️</span> Preview on Store
        </a>
...
<div class="admin-form-card">
    <form action="product_edit.php?id=<?= $product_id ?>" method="POST" enctype="multipart/form-data">
        ...
        <div class="admin-form-group">
            ...
            <input type="text" id="name" name="name" class="admin-form-control" value="<?= htmlspecialchars($name) ?>" required>
        </div>
        ...
        <div class="admin-form-row">
            ...
            <div class="admin-form-group">
                ...
                <input type="text" id="custom_category" name="custom_category" class="admin-form-control" placeholder="Enter new category name..." value="<?= htmlspecialchars($custom_category) ?>" style="margin-top: 8px; display: none;">
            </div>

            <!-- Inventory Stock -->
            <div class="admin-form-group">
                <label for="stock">Inventory Stock Quantity *</label>
                <input type="number" id="stock" name="stock" class="admin-form-control" min="0" value="<?= htmlspecialchars($stock) ?>" required>
            </div>
        </div>
        ...
        <div class="admin-form-row">
            ...
            <div class="admin-form-group">
                ...
                <input type="number" step="0.01" id="price" name="price" class="admin-form-control" value="<?= htmlspecialchars($price) ?>" required>
            </div>

            <!-- Discount Price -->
            <div class="admin-form-group">
                <label for="discount_price">Discount / Sale Price (₹) <span style="color: var(--admin-muted); font-weight: normal;">(Optional)</span></label>
                <input type="number" step="0.01" id="discount_price" name="discount_price" class="admin-form-control" value="<?= htmlspecialchars($discount_price) ?>">
            </div>
        </div>
        ...
        <div class="admin-form-group">
            ...
            <textarea id="description" name="description" class="admin-form-control" rows="4"><?= htmlspecialchars($description) ?></textarea>
        </div>

        <!-- Image URL & File Upload -->
        <div class="admin-form-row">
            <div class="admin-form-group">
                <label for="image">Image Web URL</label>
                <input type="url" id="image" name="image" class="admin-form-control" value="<?= htmlspecialchars($image) ?>" oninput="updateImagePreview(this.value)">
                <div style="font-size: 11px; color: var(--admin-muted); margin-top: 4px;">Or upload a replacement image file:</div>
            ...
            <div class="admin-form-group" style="display: flex; flex-direction: column; align-items: center; justify-content: center;">
                <label style="align-self: flex-start;">Current Image Preview</label>
